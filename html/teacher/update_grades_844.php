...
$grades_stmt = $pdo->prepare("
    SELECT subject_name, grade, final_score, paper_scores, rats_score, assessment_type, term, academic_year, is_locked, teacher_comment
                ...
                foreach ($grades_data as $subject => $paper_data) {
                    ...
                    if ($existing) {
                        $update_stmt = $pdo->prepare("
                            ...
                            WHERE id = ?
                        ");
                        $update_stmt->execute([$grade, $final_score, $grade_points, $paper_scores_json, $rats_score, $teacher['id'], $lock_submission, $teacher_comment, $existing['id']]);
                    } else {
                        $insert_stmt = $pdo->prepare("
                            INSERT INTO grades 
                            (student_id, subject_name, grade, final_score, grade_points, paper_scores, rats_score, term, assessment_type, academic_year, teacher_id, is_locked, teacher_comment) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        ");
                        $insert_stmt->execute([$student_id, $subject, $grade, $final_score, $grade_points, $paper_scores_json, $rats_score, $selected_term, $selected_assessment, $selected_year, $teacher['id'], $lock_submission, $teacher_comment]);
                    }
                }
